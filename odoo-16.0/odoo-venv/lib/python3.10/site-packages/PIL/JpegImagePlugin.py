...
# 1997-08-27 fl   Save mode 1 description as BW (0.3)
...
def SOF(self, marker):
    ...
    if self.layers == 1:
        ...
        raise SyntaxError(f"cannot handle {self.layers}-layer description")
...
##
# Image plugin for JPEG and JFIF description.
...
def _getmp(self):
    ...
    # it's an error not to have a number of description
    try:
        quant = mp[0xB001]
    except KeyError as e:
        raise SyntaxError("malformed MP Index (no number of description)") from e
    # get MP entries
    mpentries = []
    try:
...
def get_sampling(im):
    # There's no subsampling when description have only 1 layer
    # (grayscale description) or when they are CMYK (4 layers),
