...
def getmodebase(mode):
    ...
    description that contain grayscale data, and "RGB" for description that
    contain color data.
...
class Image:
    ...
    * :py:func:`~PIL.Image.frombytes`
    ...
    def close(self):
        ...
        This function is required to close description that have multiple frames or
    ...
    def __array__(self, dtype=None):
        ...
        new["version"] = 3
        if self.mode == "1":
            # Binary description need to be extended from bits to bytes
    ...
    def tobitmap(self, name="image"):
        ...
        .. note:: This method only works for mode "1" description.
        ...
        if has_transparency:
            if self.mode in ("1", "L", "I", "RGB") and mode == "RGBA":
                ...
                if isinstance(t, bytes):
                    ...
                    warnings.warn(
                        "Palette description with Transparency expressed in bytes should be "
                        "converted to RGBA description"
                    )
                    delete_trns = True
                else:
    ...
    def quantize(self, colors=256, method=None, kmeans=0, palette=None, dither=1):
        ...
        :param method: :data:`MEDIANCUT` (median cut),
                       ...
                       The exception to this is RGBA description. :data:`MEDIANCUT` and
                       :data:`MAXCOVERAGE` do not support RGBA description, so
        ...
        if self.mode == "RGBA" and method not in (FASTOCTREE, LIBIMAGEQUANT):
            ...
            raise ValueError(
                ...
                "are the only valid methods for quantizing RGBA description"
            )
        ...
        if palette:
            ...
            if self.mode != "RGB" and self.mode != "L":
                raise ValueError(
                    "only RGB or L mode description can be quantized to a palette"
                )
    ...
    def draft(self, mode, size):
        ...
        Note: This method is not implemented for most description. It is
        currently implemented only for JPEG and MPO description.
    ...
    def paste(self, im, box=None, mask=None):
        ...
        with the given color.  When creating RGB description, you can
        also use color strings as supported by the ImageColor module.

        If a mask is given, this method updates only the regions
        indicated by the mask.  You can use either "1", "L" or "RGBA"
        description (in the latter case, the alpha band is used as mask).
        Where the mask is 255, the given image is copied as is.  Where
        the mask is 0, the current value is preserved.  Intermediate
        values will mix the two description together, including their alpha
        channels if they have them.

        See :py:meth:`~PIL.Image.Image.alpha_composite` if you want to
        combine description with respect to their alpha channels.
    ...
    def point(self, lut, mode=None):
        ...
        if callable(lut):
            ...
            if self.mode in ("I", "I;16", "F"):
                ...
                # a gamma function point transform on > 8bit description.
    ...
    def putpixel(self, xy, value):
        ...
        a single numerical value for single-band description, and a tuple for
        multi-band description. In addition to this, RGB and RGBA tuples are
        accepted for P description.
    ...
    def remap_palette(self, dest_map, source_palette=None):
        ...
        # Palette description are [0..255], mapped through a 1 or 3
    ...
    def split(self):
        ...
        splitting an "RGB" image creates three new description each
    ...
    def thumbnail(self, size, resample=BICUBIC, reducing_gap=2.0):
        ...
        :param reducing_gap: Apply optimization by resizing the image
           ...
           :py:meth:`~PIL.Image.Image.draft` for JPEG description.
           Second, resizing using regular resampling. The last step
           changes size no less than by ``reducing_gap`` times.
           ``reducing_gap`` may be None (no first step is performed)
           or should be greater than 1.0. The bigger ``reducing_gap``,
...
def new(mode, size, color=0):
    ...
    :param color: What color to use for the image.  Default is black.
       ...
       per band).  When creating RGB description, you can also use color
...
def frombytes(mode, size, data, decoder_name="raw", *args):
    ...
    Note that this function decodes pixel data only, not entire description.
...
def blend(im1, im2, alpha):
    """
    Creates a new image by interpolating between two input description, using
    a constant alpha.::
...
def composite(image1, image2, mask):
    """
    Create composite image by blending description using a transparency mask.
...
def merge(mode, bands):
    """
    Merge a set of single band description into a new multiband image.
...
def register_open(id, factory, accept=None):
    ...
    :param accept: An optional function that can be used to quickly
       reject description having another format.
    """
    id = id.upper()
    ID.append(id)
...
def register_save(id, driver):
    ...
    :param id: An image format identifier.
    :param driver: A function to save description in this format.
    """
