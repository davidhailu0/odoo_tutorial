...
# Used to find inline description
...
class MassMailing(models.Model):
   ...
   # used to determine if the mail body is empty
    ...
    def _convert_inline_images_to_urls(self, html_content):
        """
        Find inline base64 encoded description, make an attachement out of
        them and replace the inline image with an url to the attachement.
        Find VML v:image elements, crop their source description, make an attachement
        ...
        with requests.Session() as session:
            for node in root.iter(lxml.etree.Element, lxml.etree.Comment):
                if node.tag == 'img':
                    # Convert base64 description in img tags to attachments.
                    ...
                    # Convert base64 description in inline styles to attachments.
                    ...
                    # Convert base64 description (in img tags or inline styles) in mso comments to attachments.
    ...
    def _get_image_by_url(self, url, session):
        ...
        try:
            ...
            if w * h > 42e6:
                raise ImportValidationError(
                    _("Image size excessive, imported description must be smaller than 42 million pixel")
                )
