...
class ProductFetchImageWizard(models.TransientModel):
    ...
    _description = "Fetch product description from Google Images based on the product's barcode number."
    ...
    def default_get(self, fields_list):
        ...
        if not ir_cron_fetch_image:
            raise UserError(_(
                "The scheduled action \"Product Images: Get product description from Google\" has "
    ...
    def action_fetch_image(self):
        """ Fetch the description of the first ten products and delegate the remaining to the cron.

        The first ten description are immediately fetched to improve the user experience. This way, they
        ...
        if self._get_products_to_process(1):  # Delegate remaining products to the cron
            ...
            message = _(
                "%(matching_images_count)s matching description have been found for %(product_count)s "
                "products.",
    ...
    def _cron_fetch_image(self):
        """ Fetch description of a list of products using their barcode.
        ...
        for product in products_to_process:
            ...
            try:
                ...
                if response.status_code == requests.codes.forbidden:
                    ...
                    _logger.warning(
                        "search quota exceeded. delegating remaining description to next cron run."
                    )
                    break
                ...
                _logger.warning(
                    "encountered too many timeouts. delegating remaining description to next cron run."
                )
                break
    ...
    def _get_image_from_url(self, url):
        ...
        """
        image = None
        if 'x-raw-image:///' not in url:  # Ignore description with incorrect link
