...
class ResUsers(models.Model):
    ...
    def _can_import_remote_urls(self):
        ...
        description via URL (as such an import can DOS a worker). By default,
                    ...
                    for num, line in enumerate(data):
                        if re.match(config.get("import_image_regex", DEFAULT_IMAGE_REGEX), line[index]):
                            if not self.env.user._can_import_remote_urls():
                                raise ImportValidationError(
                                    _("You can not import description via URL, check with your administrator or support for the reason."),
                            ...
                            try:
                                ...
                                raise ImportValidationError(
                                    _("Found invalid image data, description should be imported as either URLs or base64-encoded data."),
                                    field=name, field_type=field['type']
                                )
