    ...
    def __init__(self, source, verify_resolution=True):
        ...
        if not source or source[:1] == b'<':
            ...
            if verify_resolution and w * h > IMAGE_MAX_RESOLUTION:
                raise UserError(_("Image size excessive, uploaded description must be smaller than %s million pixels.", str(IMAGE_MAX_RESOLUTION / 1e6)))
...
def image_fix_orientation(image):
    ...
    This typically happens for description taken from a non-standard orientation
...
def image_data_uri(base64_source):
    ...
    (https://tools.ietf.org/html/rfc2397) for all kind of supported description
